"""LTX-2.3 Multimodal V2 — adds tunable audio/modality guidance scales and timed audio conditions."""
...
class LTX2_3MultiV2Plugin(ModelPlugin):
    ...
    def generate(self, pipe_obj, inputs: ModelInputs, scene, prefs) -> str:
        ...
        # ── New V2 guidance params from scene (scene_proxy set by queue_ops) ──
        _modality_scale       = getattr(scene, "ltx23m_modality_scale",       1.0)
        _audio_guidance       = getattr(scene, "ltx23m_audio_guidance",       1.0)
        _audio_stg_scale      = getattr(scene, "ltx23m_audio_stg_scale",      0.0)
        _audio_modality_scale = getattr(scene, "ltx23m_audio_modality_scale", 1.0)
        _audio_noise_scale    = getattr(scene, "ltx23m_audio_noise_scale",    0.0)
        _audio_start_time     = getattr(scene, "ltx23m_audio_start_time",     0.0)
        ...
        if image_input is not None and last_input is not None and _middle_paths:
            ...
            for _mp, _frac in _middle_paths:
                ...
                try:
                    ...
                    print(f"[LTX23MultiV2] WARNING: skipping middle anchor {_mp!r}: {_e}")
            image_conditions.append(LTX2ImageCondition(image=last_input, frame=-1, strength=1.0))
            print(f"[LTX23MultiV2] MODE: MULTI-ANCHOR — {len(image_conditions)} conditions")
        ...
        if _enabled_loras and _lora_folder:
            print(f"LTX-2.3 V2 Stage 1: loading {len(_enabled_loras)} LoRA(s)")
            self._apply_loras(pipe, _lora_folder, _enabled_loras)

        # Parse Audio Conditions — use start_time from snapshot if available
        ...
        if _enabled_loras and _lora_folder:
            print(f"LTX-2.3 V2 Stage 2: loading {len(_enabled_loras)} LoRA(s)")
            self._apply_loras(refine_pipe, _lora_folder, _enabled_loras)
        ...
        if sound_path:
            try:
                ...
                print(f"[LTX23MultiV2] Muxing input audio: {_sr} Hz, {_wav.shape[-1]} samples → {dur_s:.2f}s")
            except Exception as _ae:
                print(f"[LTX23MultiV2] Input audio mux failed ({_ae}), falling back to model audio.")
        ...
        print(f"LTX-2.3 V2 saved: {dst_path}")
        return dst_path
